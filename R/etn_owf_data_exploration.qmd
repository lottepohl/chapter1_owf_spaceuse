...
```{r etn-dp-sql}
# get deployment ids in the right format for the SQL query
deployment_ids_sql <- 
  paste0("('", paste(deployments %>% dplyr::filter(inside_OWF == TRUE) %>% dplyr::select(deployment_id) %>% unique(), collapse = "', '"), "')")

library(etn)
packageVersion('etn')
detections <- 
  etn::get_acoustic_detections(deployment_id = 3336)
